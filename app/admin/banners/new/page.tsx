...
export default function NewBannerPage() {
  ...
  const validate = () => {
    ...
    if (!formData.image) newErrors.image = "Desktop image is required";
 ...
 const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    const uploadToCloudinary = async (file: File, label: string) => {
      ...
      if (!res.ok) {
        throw new Error(
          uploaded?.error?.message || `${label} image upload failed`
        );
      }
    ...
    const imageUrl =
      formData.image instanceof File
        ? await uploadToCloudinary(formData.image, "Desktop")
        : formData.image;

    const mobileImageUrl =
      formData.mobileImage instanceof File
        ? await uploadToCloudinary(formData.mobileImage, "Mobile")
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className={`block w-full text-sm border p-2 rounded ${
                    errors.mobileImage ? "border-red-500" : ""
                  }`}
                />
